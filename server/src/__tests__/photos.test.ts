...
describe('GET /api/photos by location_id', () => {
  it('returns photos from all bins in a location', async () => {
    const { token } = await createTestUser(app);
    const location = await createTestLocation(app, token);
    const bin1 = await createTestBin(app, token, location.id);
    const bin2 = await createTestBin(app, token, location.id);

    await uploadPhoto(app, token, bin1.id);
    await uploadPhoto(app, token, bin2.id);
  ...
  it('respects the limit query parameter', async () => {
    ...
    const res = await request(app)
      .get('/api/photos')
      .query({ location_id: location.id, limit: 1 })
      .set('Authorization', `Bearer ${token}`);
  ...
  it('excludes photos from soft-deleted bins', async () => {
    const { token } = await createTestUser(app);
    const location = await createTestLocation(app, token);
    const bin1 = await createTestBin(app, token, location.id);
    const bin2 = await createTestBin(app, token, location.id);
    ...
    // Soft-delete bin2 via the API
    await request(app)
      .delete(`/api/bins/${bin2.id}`)
      .set('Authorization', `Bearer ${token}`);
    ...
    expect(res.status).toBe(200);
    expect(res.body.results.length).toBe(1);
    expect(res.body.results[0].bin_id).toBe(bin1.id);
  });

  it('returns 403 when the user is not a member of the location', async () => {
    const { token } = await createTestUser(app);
    const location = await createTestLocation(app, token);
